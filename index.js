...
const Open_API_Url =
  process.env.Open_API_Url || "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key=";
...
app.get("/", async (_, res) => {
  try {
    res.status(200).json({ message: "Welcome to codeAIx " });
  } catch (error) {
...
app.post("/run", async (req, res) => {
  try {
    ...
    const responseData = response?.data?.contents[0].parts[0].text;
